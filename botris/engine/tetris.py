...
class TetrisGame:
    ...
    @classmethod
    def from_game_state(cls, game_state: GameState, options: dict[str, Any] | None=None) -> TetrisGame:
        """
        Creates a Tetris game instance from a given game state.
    ...
    def execute_move(self, move: Move) -> list[Event]:
        ...
        match move:
            case Move.move_left:
                test_piece: PieceData | None = move_left(self.board, self.current, self.options.board_width)
                if test_piece is not None:
                    self.current = test_piece
            case Move.move_right:
                test_piece: PieceData | None = move_right(self.board, self.current, self.options.board_width)
                if test_piece is not None:
                    self.current = test_piece
            case Move.drop:
                test_piece: PieceData | None = move_drop(self.board, self.current, self.options.board_width)
                if test_piece is not None:
                    self.current = test_piece
            ...
            case Move.sonic_drop:
                ...
                self.current = test_piece
            case Move.rotate_cw:
                test_piece: PieceData | None = rotate_cw(self.board, self.current, self.options.board_width)
                if test_piece is not None:
                    self.current = test_piece
                    self.is_immobile = check_immobile(self.board, self.current, self.options.board_width)
            case Move.rotate_ccw:
                test_piece: PieceData | None = rotate_ccw(self.board, self.current, self.options.board_width)
                if test_piece is not None:
                    self.current = test_piece
                    self.is_immobile = check_immobile(self.board, self.current, self.options.board_width)
            case Move.hold:
