...
def generate_moves(board: Board, piece: Piece, alternative: Optional[Piece], board_height: int, board_width: int, algo: Literal['bfs', 'dfs', 'dijk', 'dijk-short']='bfs') -> Dict[PieceData, List[Move]]:
    match algo:
        case 'bfs':
...
def dfs_generate_moves(board: Board, piece: Piece, alternative: Optional[Piece], board_height: int, board_width: int) -> Dict[PieceData, List[Move]]:
...
def bfs_generate_moves(board: Board, piece: Piece, alternative: Optional[Piece], board_height: int, board_width: int) -> Dict[PieceData, List[Move]]:
...
def short_generate_moves(board: Board, piece: Piece, alternative: Optional[Piece], board_height: int, board_width: int) -> Dict[PieceData, List[Move]]:
...
def dijkstra_generate_moves(board: Board, piece: Piece, alternative: Optional[Piece], board_height: int, board_width: int) -> Dict[PieceData, List[Move]]:
...
def dijkstra_generate_moves_short(board: Board, piece: Piece, alternative: Optional[Piece], board_height: int, board_width: int) -> Dict[PieceData, List[Move]]:
...
def add_move(generated_moves: Dict[PieceData, List[Move]], piece: PieceData, move: List[Move]) -> None:
    if (piece not in generated_moves) or (len(move) < len(generated_moves[piece])):
        generated_moves[piece] = move


def dfs_generate_move_helper(board: Board, current_piece: PieceData, generated_moves: Dict[PieceData, List[Move]], board_height: int, board_width: int, move: List[Move], visited: Set[PieceData]) -> None:
    ...
    if move_drop_piece is None:
        ...
        dfs_generate_move_helper(board, move_drop_piece, generated_moves, board_height, board_width, move + [Move.drop], visited)

    move_left_piece: Optional[PieceData] = move_left(board, current_piece, board_width)
    if move_left_piece is not None:
        dfs_generate_move_helper(board, move_left_piece, generated_moves, board_height, board_width, move + [Move.left], visited)
    move_right_piece: Optional[PieceData] = move_right(board, current_piece, board_width)
    if move_right_piece is not None:
        dfs_generate_move_helper(board, move_right_piece, generated_moves, board_height, board_width, move + [Move.right], visited)

    rotate_cw_piece: Optional[PieceData] = rotate_cw(board, current_piece, board_width)
    if rotate_cw_piece is not None:
        dfs_generate_move_helper(board, rotate_cw_piece, generated_moves, board_height, board_width, move + [Move.rotate_cw], visited)

    rotate_ccw_piece: Optional[PieceData] = rotate_ccw(board, current_piece, board_width)
    if rotate_ccw_piece is not None:
        dfs_generate_move_helper(board, rotate_ccw_piece, generated_moves, board_height, board_width, move + [Move.rotate_ccw], visited)

def bfs_generate_move_helper(board: Board, current_piece: PieceData, generated_moves: Dict[PieceData, List[Move]], board_height: int, board_width: int, held: bool=False) -> None:
    queue: Deque[Tuple[PieceData, List[Move]]] = [(current_piece, [Move.hold] if held else [])]
    visited: Set[PieceData] = set()
...
def dijkstra_generate_move_helper(board: Board, current_piece: PieceData, generated_moves: Dict[PieceData, List[Move]], board_height: int, board_width: int, held: bool=False) -> None:
    priority_queue: List[Tuple[int, PieceData, List[Move]]] = [(0, current_piece, ['hold'] if held else [])]
    visited: Set[PieceData] = set()
    ...
    while priority_queue:
        ...
        if move_drop_piece is not None:
            ...
            if move_drop_piece not in distance or new_distance < distance[move_drop_piece]:
                ...
                heappush(priority_queue, (new_distance, move_drop_piece, move + [Move.drop]))
        else:
            add_move(generated_moves, current_piece, move)
        ...
        if move_left_piece is not None:
            ...
            if move_left_piece not in distance or new_distance < distance[move_left_piece]:
                ...
                heappush(priority_queue, (new_distance, move_left_piece, move + [Move.left]))
        ...
        if move_right_piece is not None:
            ...
            if move_right_piece not in distance or new_distance < distance[move_right_piece]:
                ...
                heappush(priority_queue, (new_distance, move_right_piece, move + [Move.right]))
        ...
        if rotate_cw_piece is not None:
            ...
            if rotate_cw_piece not in distance or new_distance < distance[rotate_cw_piece]:
                ...
                heappush(priority_queue, (new_distance, rotate_cw_piece, move + [Move.rotate_cw]))
        ...
        if rotate_ccw_piece is not None:
            ...
            if rotate_ccw_piece not in distance or new_distance < distance[rotate_ccw_piece]:
                ...
                heappush(priority_queue, (new_distance, rotate_ccw_piece, move + [Move.rotate_ccw]))


def short_dijkstra_generate_move_helper(board: Board, current_piece: PieceData, generated_moves: Dict[PieceData, List[Move]], board_height: int, board_width: int, held: bool=False):
    priority_queue: List[Tuple[int, PieceData, List[Move]]] = [(0, current_piece, [Move.hold] if held else [])]
    visited: Set[PieceData] = set()
    distance: Dict[PieceData, int] = {current_piece: 0}
        ...
        if move_drop_piece is not None:
            ...
            if sonic_drop_piece is not None:
                ...
                if sonic_drop_piece not in distance or new_distance < distance[sonic_drop_piece]:
                    ...
                    heappush(priority_queue, (new_distance, sonic_drop_piece, move + [Move.sonic_drop]))
        ...
        if move_left_piece is not None:
            ...
            if sonic_left_piece is not None:
                ...
                if sonic_left_piece not in distance or new_distance < distance[sonic_left_piece]:
                    ...
                    heappush(priority_queue, (new_distance, sonic_left_piece, move + [Move.sonic_left]))

            if move_left_piece not in distance or new_distance < distance[move_left_piece]:
                distance[move_left_piece] = new_distance
                heappush(priority_queue, (new_distance, move_left_piece, move + [Move.move_left]))
        ...
        if move_right_piece is not None:
            ...
            if sonic_right_piece is not None:
                ...
                if sonic_right_piece not in distance or new_distance < distance[sonic_right_piece]:
                    ...
                    heappush(priority_queue, (new_distance, sonic_right_piece, move + [Move.sonic_right]))
            if move_right_piece not in distance or new_distance < distance[move_right_piece]:
                distance[move_right_piece] = new_distance
                heappush(priority_queue, (new_distance, move_right_piece, move + [Move.move_right]))
...
def short_generate_move_helper(board: Board, current_piece: PieceData, generated_moves: Dict[PieceData, List[Move]], board_height: int, board_width: int, held: bool=False) -> None:
    queue: Deque[Tuple[PieceData, List[Move]]] = [(current_piece, [Move.hold] if held else [])]
    visited: Set[PieceData] = set()
